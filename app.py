...
DEFAULT_MODEL = os.getenv("OPENAI_MODEL", "gpt-4o-mini")  # override via env var
...
# NHS number: 10 digits, usually spaced 3-3-4; heuristic
...
def detect_ae_or_pii(text: str) -> Tuple[bool, List[str]]:
    ...
    # Phone is noisy; still block if found (conservative posture)
...
# =========================
# Central helper for blocked rendering (NEW)
# =========================

def render_blocked(reasons: List[str], extra: Optional[str] = None) -> None:
    """
    Renders a consistent blocked warning box.
    Uses html.escape (safe + Streamlit-compatible) instead of removed internal APIs.
    """
    reason_lines = "<br/>".join([html.escape(r) for r in reasons]) if reasons else html.escape("Safety trigger.")
...
def generate_json(
    ...
    - Strongly instructs "Return JSON only".
    - No references beyond user input (enforced by prompt + MI tool constraints).
    """
...
def render_copy_button(text: str, key: str) -> None:
    ...
    """
    st.components.v1.html(html_block, height=55)
...
def render_output_block(tool_title: str, result: Dict[str, Any], key_prefix: str) -> None:
    ...
    - JSON viewer
    - "pretty text" bundle for clipboard/PDF export
    """
    if not result:
        return
    ...
    if result.get("blocked") is True:
        render_blocked(
            [str(result.get("reason", "Safety trigger."))],
            extra=str(result.get("safe_next_step", "Remove AE/PII and try again.")),
        )
        st.json(result)
        return
...
def sidebar_nav() -> Tuple[str, str, float]:
    ...
    _ = st.sidebar.radio("Navigate", options=["Home"], label_visibility="collapsed", key="nav_home")
    ...
    # If a tile click set a target, honour it once then clear.
    if st.session_state.get("tile_target"):
        selected = st.session_state["tile_target"]
        st.session_state["tile_target"] = None
        # Adjust group
        if selected in [
            "Scientific Narrative Generator",
            "MSL Briefing Pack Generator",
    ...
    # Default behaviour: keep last group selected
    if st.session_state.selected_page_group == "Core":
        selected = core_choice
    elif st.session_state.selected_page_group == "Additional":
        selected = add_choice
    else:
        selected = "Home"
...
def tile_button(label: str, description: str, page_name: str, icon: str) -> None:
    ...
    if st.button(f"Open: {label}", key=f"tile_{page_name}", use_container_width=True):
        st.session_state["tile_target"] = page_name
        st.session_state.selected_page = page_name
        if TOOL_DEFS[page_name]["group"] == "Core Medical Affairs Tools":
            st.session_state.selected_page_group = "Core"
        elif TOOL_DEFS[page_name]["group"] == "Additional Tools":
            st.session_state.selected_page_group = "Additional"
        else:
            st.session_state.selected_page_group = "Home"
        st.rerun()
...
def page_scientific_narrative(model: str, temperature: float) -> None:
    ...
    if run:
        if blocked:
            render_blocked(reasons, extra="Remove AE / patient-identifiable data and try again.")
            return
...
def page_msl_briefing_pack(model: str, temperature: float) -> None:
    ...
    with st.form("msl_form"):
        ...
        constraints = st.text_area(
            "Constraints / considerations (geography, setting, compliance notes)",
            height=110,
            value="UK & Ireland default. Non-promotional. Scientific exchange only."
        )
...
def page_mi_response(model: str, temperature: float) -> None:
    ...
    with st.form("mi_form"):
        ...
        question = st.text_area(
            "Medical question",
            height=120,
            placeholder="Enter the medical question exactly as received (no patient identifiers)."
        )
        evidence = st.text_area(
            "Evidence provided by user (paste excerpts, citations, or internal text)",
            height=180,
            help="References MUST be included here if you want them cited."
        )
    ...
    if run:
        if blocked:
            render_blocked(
                reasons,
                extra="This tool cannot process adverse events (PV) or patient-identifiable data. Remove it and try again."
            )
            return
...
def page_congress_adboard(model: str, temperature: float) -> None:
    ...
    with st.form("cab_form"):
        ...
        constraints = st.text_area(
            ...
            height=100,
            value="Non-promotional. Scientific exchange. Ensure appropriate compliance review."
        )
...
def page_insight_analysis(model: str, temperature: float) -> None:
    ...
    if not st.session_state.insights:
        ...
        for i, item in enumerate(st.session_state.insights, start=1):
            safe_line = html.escape(item["insight"])
            safe_meta = html.escape(f"{item['stakeholder_type']} • {item['region']}")
            st.markdown(
                f"<div class='pd-card'><b>{i}. {safe_meta}</b><br/>{safe_line}</div>",
                unsafe_allow_html=True,
            )
    ...
    if run:
        ...
        blocked, reasons = detect_ae_or_pii(combined)
        if blocked:
            render_blocked(reasons, extra="Remove AE / patient-identifiable data from insights and try again.")
            return
...
def page_exec_summary(model: str, temperature: float) -> None:
    ...
    with st.form("execsum_form"):
        ...
        strategic_focus = st.text_input(
            "Strategic focus",
            placeholder="e.g., H2 priorities, KOL strategy, evidence-generation, congress planning"
        )
...
def page_sop_drafting(model: str, temperature: float) -> None:
    ...
    with st.form("sop_form"):
        ...
        existing = st.text_area(
            ...
            height=180,
            placeholder="Paste existing SOP text if available (remove any patient-identifiable content)."
        )
        extra_requirements = st.text_area(
            "Additional requirements (optional)",
            height=110,
            placeholder="E.g., required sections, approval workflow, training cadence."
        )
